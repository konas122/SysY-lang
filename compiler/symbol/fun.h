...
class Fun : public std::enable_shared_from_this<Fun>
{
    // 基本信息
    bool externed;              // 声明或定义
    Tag type;                   // 变量类型
    std::string name;           // 变量名称
    std::vector<std::shared_ptr<Var>> paraVar; // 形参变量列表
...
public:
    Fun(bool ext, Tag t, std::string_view n, const std::vector<std::shared_ptr<Var>> &paraList);
    ~Fun();

    // 声明定义与使用
    bool match(std::shared_ptr<Fun> f);                         // 声明定义匹配
    bool match(const std::vector<std::shared_ptr<Var>> &args); // 行参实参匹配
    void define(const std::shared_ptr<Fun> def);                // 将函数声明转换为定义, 需要拷贝参数列表, 设定 extern

    // 作用域管理, 局部变量地址计算
    void enterScope();      // 进入一个新的作用域
    void leaveScope();      // 离开当前作用域
    void locate(std::shared_ptr<Var> var);  // 定位局部变了栈帧偏移
    ...
    void optimize(std::shared_ptr<SymTab> tab);             // 执行优化操作
    ...
    std::vector<std::shared_ptr<Var>> &getParaVar();   // 获取参数列表, 用于为参数生成加载代码
